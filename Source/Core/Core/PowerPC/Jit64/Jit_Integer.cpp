...
void Jit64::ComputeRC(preg_t preg, bool needs_test, bool needs_sext)
{
  ...
  if (CheckMergedBranch(0))
  {
    ...
    {
      // If the output operand to the cmp/rc op we're merging with the branch isn't used anymore,
      // it'd be better to flush it here so that we don't have to flush it on both sides of the
      // branch. The flush is before the TEST so that it can macro-op fusion with the conditional
      // branch.
...
void Jit64::DoMergedBranchCondition()
{
  ...
  if (!IsInBlockBranchActive())
  {
    ...
    gpr.Flush(~js.op->gprWillBeWritten & js.op->regsOut);
  }
