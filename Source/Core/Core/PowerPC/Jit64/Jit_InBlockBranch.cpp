...
void Jit64::ForcePreloadRegisters()
{
  ...
  RCX64Reg scratch_guard = gpr.Scratch(RSCRATCH_EXTRA, false);
...
bool Jit64::TryPrepareInBlockBranches(const PPCAnalyst::CodeOp& op, const BitSet32 locked_gprs)
{
  const auto AreThereEnoughHostRegs = [&](RegsUsed regs) {
    return (regs.regsIn | regs.regsOut | locked_gprs).Count() <= gpr.GetMaxPreloadableRegisters() &&
  ...
  const auto HandleBarriers = [&]() {
    // TODO: If previous instruction(s) end the block, then don't emit anything.
    ...
    for (const PPCAnalyst::BranchInfo& bi : code_block.m_branch_infos)
    {
      ...
      {
        first_barrier = false;
        ForcePreloadRegisters();  // TODO: Bad idea, again, in use for the Analyzer would
                                  // have been better.
        DecreaseDowncount();
  ...
  const auto FlushUnusedRegisters = [&]() {
    gpr.Flush(~(js.inBlockBranchStatus.regsIn | js.inBlockBranchStatus.regsOut | locked_gprs));
    const BitSet32 regsToFinishFlushing =
        locked_gprs & ~(js.inBlockBranchStatus.regsIn | js.inBlockBranchStatus.regsOut);
    gpr.Flush(regsToFinishFlushing, RegCache::FlushMode::MaintainState);
    js.inBlockBranchStatus.regsToFinishFlushing = regsToFinishFlushing;
  ...
  if (js.inBlockBranchStatus.ends_at.has_value())
  {
    ...
    if (op.i >= js.inBlockBranchStatus.ends_at.value())
    {
      ...
      gpr.Flush(~op.gprInUse, RegCache::FlushMode::Full, RegCache::IgnoreDiscardedRegisters::Yes);
      fpr.Flush(~op.fprInUse, RegCache::FlushMode::Full, RegCache::IgnoreDiscardedRegisters::Yes);
  ...
  else
  {
    ...
    else
    {
      ...
      //           << "Which starts at index " << std::dec << it->address_i << " and ends at index "
      ...
      // TODO: Should be added by the Analyzer in contains_flush_and_continue
...
Jit64::TryInBlockBranch(const PPCAnalyst::CodeOp& op, const BitSet32 locked_gprs)
{
  // ASSERT(op.branchTo != UINT32_MAX);

  // For merged branches, conclude the flushing.
  gpr.Flush(js.inBlockBranchStatus.regsToFinishFlushing);
  js.inBlockBranchStatus.regsToFinishFlushing = {};
  ...
  if (TryPrepareInBlockBranches(op, locked_gprs))
  {
