...
void SystemTimersManager::VICallback(Core::System& system, u64 userdata, s64 cycles_late)
{
  ...
  if (Config::Get(Config::MAIN_VI_OVERCLOCK_ENABLE))
  {
    ...
    core_timing.ScheduleEvent(ticks_per_halfline_overclocked - cycles_late, system.GetSystemTimers().m_event_type_vi);
  }
  else
  {
    core_timing.ScheduleEvent(ticks_per_halfline - cycles_late, system.GetSystemTimers().m_event_type_vi);
  }
}
