                                             ...
                                             u32 num_blocks_y)
{
  ...
  {
    std::memset(dst, 0, bytes_per_row * num_blocks_y);
  }
  else
  {
    u8* ptr = dst;
...
//
// An alternative would be using std::wmemset, but in testing it seems to perform slightly worse
// (and changes nothing on MSVC, where on x86 it emits `rep stosw`, and on ARM a naive loop).
//
// Clang is excluded here becuase it's already aggressive at -O2.
//
                                             ...
                                             u32 num_blocks_y)
{
  ...
  if (bytes_per_row == stride)
  {
    ...
    // On MSVC, std::ranges::fill_n prefers a naive loop. This instead prefers `rep stosw` (on x86).
