...
static u32 ExpectedContentForWindowsBoundary(size_t index)
{
  switch (index)
  {
  case Common::LazyMemoryRegion::WINDOWS_BLOCK_SIZE / 4 - 1:
    return 0x80000000;
  case Common::LazyMemoryRegion::WINDOWS_BLOCK_SIZE / 4:
    return 0x00000001;
  default:
    return 0;
  }
...
{
  const size_t end_index = Common::AlignUp(bits, 32) / 32;
  for (size_t i = 0; i < end_index; ++i, ++memory)
  {
    u32 expected = f(i);
    if (i == end_index - 1)
      expected &= 0xFFFFFFFF >> (32 - bits % 32);
